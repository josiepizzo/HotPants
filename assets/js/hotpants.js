function guardian() {
    ...
    $.ajax({ url: queryURL, method: 'GET' }).done(function(response) {
        ...
        $("#guardianDisplay").append("<h2 id='guardianHeader'>Latest News from " + "<img src='assets/images/guardian1.gif' alt='guardianLogo'>" + "</h2>")
...
function giphy() {
    ...
    $.ajax({ url: queryURL, method: 'GET' }).done(function(response) {
        ...
        for (var i = 0; i < response.data.length; i++) {
            ...
            var poliGif = $('<img id="gifDisplay">').attr("src", response.data[i].images.fixed_height.url);
